...
def _hash_file_as_posix(file: Path, *, cwd: Path) -> str:
...
def hash_directory(path: str, *, cwd: str = None) -> list[str]:
    ...
    # with Thread/ProcessPoolExecutor() as executor:
    digests = map(lambda file: _hash_file_as_posix(file, cwd=cwd), files)
